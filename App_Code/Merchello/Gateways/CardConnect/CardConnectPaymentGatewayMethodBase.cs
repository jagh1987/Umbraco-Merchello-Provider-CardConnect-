...
namespace Merchello.Plugin.Payments.CardConnect.Provider
{
    ...
    public abstract class CardConnectPaymentGatewayMethodBase : PaymentGatewayMethodBase, ICardConnectPaymentGatewayMethod
    {
        protected CardConnectPaymentProcessorBase _processor;
        public CardConnectPaymentGatewayMethodBase(IGatewayProviderService gatewayProviderService, IPaymentMethod paymentMethod)
        ...
        protected override IPaymentResult PerformCapturePayment(IInvoice invoice, IPayment payment, decimal amount, ProcessorArgumentCollection args)
        {
            ...
            if (!result.Payment.Success)
            {
                ...
                GatewayProviderService.ApplyPaymentToInvoice(payment.Key, invoice.Key, AppliedPaymentType.Denied, "CardConnect: request capture error: " + result.Payment.Exception.Message, 0);
            }
            else
            {
                GatewayProviderService.Save(payment);
                GatewayProviderService.ApplyPaymentToInvoice(payment.Key, invoice.Key, AppliedPaymentType.Debit, "CardConnect: captured", amount);
